...
async function main() {
  ...
  const localBin = path.join(__dirname, 'node_modules', '.bin', binName);
    ...
    if (process.platform === 'win32') {
      if (hasLocal) {
        ...
        staticSrv = spawn('cmd.exe', ['/c', localBin, '-c-1', '-p', String(staticPort)], { stdio: 'inherit', cwd: __dirname });
      } else {
        // fallback to npx.cmd
        staticSrv = spawn('cmd.exe', ['/c', 'npx.cmd', 'http-server', '-c-1', '-p', String(staticPort)], { stdio: 'inherit', cwd: __dirname });
      }
    } else {
      const staticCmd = hasLocal ? localBin : 'npx';
      const staticArgs = hasLocal ? ['-c-1','-p', String(staticPort)] : ['http-server','-c-1','-p', String(staticPort)];
  ...
  const url = `http://127.0.0.1:${staticPort}/src/pages/login.html`;
