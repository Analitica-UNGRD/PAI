...
function calcularResumenIndicadores() {
  try {
    ...
    // Procesar actividades
    actividades.forEach(actividad => {
      const indicadorId = actividad.indicador_id ? actividad.indicador_id.toString().trim() : '';
      const indicadorTexto = (actividad.indicador_detalle || actividad.indicador || actividad.indicadorNombre || '').toString().trim();

      if (indicadorId && resumenIndicadores[indicadorId]) {
        const resumen = resumenIndicadores[indicadorId];
      ...
      const textoKey = `texto::${normalizeText(indicadorTexto)}`;
      if (!resumenIndicadores[textoKey]) {
        resumenIndicadores[textoKey] = {
          codigo: textoKey,
          nombre: indicadorTexto,
          totalActividades: 0,
          metaCumplida: 0,
          enProceso: 0,
          esIndicadorLibre: true
        };
      } else if (!resumenIndicadores[textoKey].nombre) {
        resumenIndicadores[textoKey].nombre = indicadorTexto;
      }

      const resumenLibre = resumenIndicadores[textoKey];
      resumenLibre.totalActividades++;

      if (actividad.estado === 'Completada') {
        resumenLibre.metaCumplida++;
      } else if (actividad.estado === 'En Progreso') {
        resumenLibre.enProceso++;
      }
    });
