...
function processRequest(request) {
  try {
    ...
    // Obtener handler para la ruta de manera simple
    let response;
    
    // Rutas básicas sin delegación compleja
    ...
    } else if (path === 'getCatalogos') {
      response = getCatalogosAgrupados();
    // Rutas de autenticación locales (login, createUser, validate)
    } else if (path === 'login' || path === 'auth/login' ||
               path === 'createUser' || path === 'auth/createUser' ||
               path === 'validate' || path === 'auth/validate') {
      // Delegar a los handlers locales (auth)
      response = handleLocalRoutes({ path: path, payload: payload });
...
function handleCreateUser(payload) {
  try {
    const { email, password, role = 'contratista' } = payload;
    ...
    return formatResponse(true, { email: result.email }, 'Usuario creado exitosamente');
