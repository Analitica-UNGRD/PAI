...
export function renderSidebar(target, options = {}) {
  ...
  const obtenerRolActual = () => {
    try {
      ...
      const normalizado = normalizarRol(stored);
      return normalizado || 'visualizador';
    } catch (error) {
      return 'visualizador';
    }
  };

  const rolUsuario = obtenerRolActual();
