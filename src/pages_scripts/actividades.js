...
const DEFAULT_APPS_SCRIPT = 'https://script.google.com/macros/s/AKfycby1o5VR8WdaQDFtcABV7vzyQuUIMGQKkp0VCoyJQbUJREBZ3MPNqszLeXGz_UA17nPNCw/exec';
const DEFAULT_DEV_PROXY = 'http://localhost:3000/api';

function resolveScriptUrl() {
  try {
    // 1) override manual inyectado en window (start-all.js / login.html)
    if (typeof window !== 'undefined' && window.APP_CONFIG_OVERRIDE && window.APP_CONFIG_OVERRIDE.BASE_URL) {
      console.log('[DEBUG] resolveScriptUrl: Usando override manual:', window.APP_CONFIG_OVERRIDE.BASE_URL);
      return window.APP_CONFIG_OVERRIDE.BASE_URL;
    }
    // 2) Si estamos ejecutando en entorno local (localhost o 127.0.0.1),
    // usar automáticamente el proxy de desarrollo para evitar problemas CORS.
    if (typeof window !== 'undefined' && window.location && window.location.hostname) {
      const h = window.location.hostname;
      if (h === 'localhost' || h === '127.0.0.1' || h === '0.0.0.0') {
      console.log('[INFO] resolveScriptUrl: Detectado entorno local, usando proxy de desarrollo:', DEFAULT_DEV_PROXY);
        return DEFAULT_DEV_PROXY;
      }
    }

    // 3) Por defecto (producción), usar Apps Script público
  console.log('[INFO] resolveScriptUrl: Usando Google Apps Script:', DEFAULT_APPS_SCRIPT);

  } catch (e) {
  console.warn('[WARN] resolveScriptUrl: Error en lógica, usando fallback:', e);
  }

  // 3) fallback a Apps Script público
...
console.log('[INFO] CONFIG_BACKEND: Para usar proxy local, ejecuta: localStorage.setItem("USE_LOCAL_PROXY", "true")');
...
class ActividadesManager {
  ...
  mostrarError(mensaje) {
    ...
    alert(mensaje); // Simplificado - en producción usar mejor UX
  }
