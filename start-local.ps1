...
$tryUrls = @(
    ...
    'http://127.0.0.1:8081/pages/login.html',
    'http://127.0.0.1:8082/pages/login.html'
)
...
if (-not (Test-CommandExists -Name 'node')) {
    ...
    Write-Host "Sugerencias para Windows:"
    Write-Host " - Instale Node.js desde https://nodejs.org/ (LTS recomendado)."
    Write-Host " - O instale con Winget: winget install OpenJS.NodeJS.LTS" -ForegroundColor Yellow
    Write-Host " - O instale con Chocolatey (si lo tiene): choco install nodejs-lts" -ForegroundColor Yellow
    Write-Host " - Si ya lo instalo, cierre y reabra PowerShell o agregue la carpeta de Node (p.ej. C:\Program Files\nodejs) a su PATH."
    Write-Host "Verifique con: node -v" -ForegroundColor Cyan
    exit 1
}
...
if (-not (Test-CommandExists -Name 'npm')) {
    ...
    Write-Host "Verifique con: npm -v" -ForegroundColor Cyan
    Write-Host "Si npm falta pero node existe, reinstale Node.js o instale npm separadamente." -ForegroundColor Yellow
    exit 1
}
...
# Prefer the npm.cmd executable on Windows PowerShell to avoid execution policy issues with npm.ps1
try {
    $npmCmdObj = Get-Command -Name 'npm.cmd' -ErrorAction SilentlyContinue
    if ($npmCmdObj) { $npmExePath = $npmCmdObj.Path } else { $npmExePath = (Get-Command -Name 'npm' -ErrorAction SilentlyContinue).Path }
} catch {
    $npmExePath = 'npm'
}
...
if ($runInline) {
    ...
    # Ejecutar npm de forma robusta: prefiera npm.cmd cuando exista (evita bloqueos por ExecutionPolicy)
    if ($npmExePath) { Start-Process -FilePath $npmExePath -ArgumentList 'run','start' -NoNewWindow -Wait } else { Start-Process -FilePath 'npm' -ArgumentList 'run','start' -NoNewWindow -Wait }
    ...
    # Fallback: ejecutar npm de forma robusta (preferir npm.cmd)
    if ($npmExePath) { Start-Process -FilePath $npmExePath -ArgumentList 'run','start' -NoNewWindow -Wait } else { Start-Process -FilePath 'npm' -ArgumentList 'run','start' -NoNewWindow -Wait }
    Write-Host 'Presione Enter para cerrar esta ventana...'
    Read-Host
        return
